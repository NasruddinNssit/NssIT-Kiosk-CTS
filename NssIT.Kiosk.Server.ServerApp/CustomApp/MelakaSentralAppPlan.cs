...
{
    /// <summary>
    /// This plan also valid for Gombak
    /// </summary>
    public class MelakaSentralAppPlan : IServerAppPlan
    {
        private CultureInfo _provider = CultureInfo.InvariantCulture;

        private AppGroup _appGroup = AppGroup.MelakaSentral;

        public MelakaSentralAppPlan()
        {
            _appGroup = NssIT.Kiosk.AppDecorator.Config.Setting.GetSetting().ApplicationGroup;
        }
        ...
        public UISalesInst NextInstruction(string procId, Guid? netProcId, IKioskMsg svcMsg, UserSession session, out bool releaseSeatRequestOnEdit)
        {
            ...
            if (svcMsg != null)
            {
                if (svcMsg is UIDetailEditRequest uiEdit)
                {
                    ...
                    if (uiEdit.EditItemCode == TickSalesMenuItemCode.ToStation)
                    {
        ...
        public UserSession UpdateUserSession(UserSession userSession, IKioskMsg svcMsg)
        {
            if (svcMsg is UILanguageSubmission uiLang)
            {
                ...
                if (_appGroup == AppGroup.Gombak)
                {
                    // Gombak Custom Setting
                    userSession.OriginStationCode = AppStationCode.Gombak;
                    userSession.OriginStationName = "TERMINAL BERSEPADU GOMBAK";
                }
                else
                {
                    // Melaka Custom Setting
                    userSession.OriginStationCode = AppStationCode.MelakaSentral;
                    userSession.OriginStationName = "MELAKA SENTRAL";
                }
