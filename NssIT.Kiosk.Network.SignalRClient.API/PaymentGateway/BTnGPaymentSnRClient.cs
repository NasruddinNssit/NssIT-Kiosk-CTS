...
namespace NssIT.Kiosk.Network.SignalRClient.API.PaymentGateway
{
    ...
    public class BTnGPaymentSnRClient : IDisposable 
    {
        ...
        public BTnGPaymentSnRClient(string webApiUrl, string deviceId, string machineCode, string docNo, decimal paymentAmount, string current, 
            string paymentGateway, string firstName, string lastName, string contactNo, string financePaymentMethod)
        {
            ...
            NssIT.Kiosk.AppDecorator.Common.AppStationCode.GetStationInfo(Setting.GetSetting().ApplicationGroup, out string stationCode, out _);
        ...
        private async void SaleThreadWorking()
        {
                ...
                while (_disposed == false)
                {
                    ...
                    if ((debugReconnect) || (_realTimeConnection == null) || (_realTimeConnection.State == HubConnectionState.Reconnecting) || (_realTimeConnection.State == HubConnectionState.Disconnected))
                    {
                        ...
                        if (conRest.IsConnected)
                        {
                            ...
                            await _realTimeConnection.InvokeAsync<string>("RegisterMachine", _deviceId, _machineCode, _stationCode);
                            ...
                            if (_paymentRequestResponse is null)
                            {
                                ...
                                string paymentRequestResult = await CreatePaymentTransaction(_deviceId, _machineCode, _docNo, _paymentAmount, _current, _paymentGateway, _firstName, _lastName, _contactNo, _stationCode, 
                                    _financePaymentMethod, _snrConnectionId);
            ...
            async Task<string> CreatePaymentTransaction(string deviceIdX, string machineCodeX, 
                ...
                string firstNameX, string lastNameX, string contactNoX, string stationCodeX,
                ...
                string snrConnectionId)
            {
                ...
                string resultStr = await _realTimeConnection.InvokeAsync<string>("CreateSalePayment", reqStr, _stationCode);
                return resultStr;
            }
                        ...
                        try
                        {
                            ...
                            else if (resultX == PaymentResult.Timeout)
                            {
                                ...
                                _log?.LogError(LogChannel, _docNo, new Exception($@"PaymentResult: {Enum.GetName(typeof(PaymentResult), resultX)}; {errorX?.Message}; (EXIT35.0107.X01)", errorX),
                                    "X10", "BTnGPaymentGatewaySnRClient.RaiseOnPaymentCompleted", adminMsg: $@"Error found when ending transaction; {errorX?.Message}; (EXIT35.0107.X01)") ;
                        }
                        catch (Exception ex2)
                        {
                            _log?.LogError(LogChannel, _docNo, new Exception($@"Unhandled event exception. {ex2.Message}; (EXIT35.0107.EX01)", ex2), "EX01", "BTnGPaymentGatewaySnRClient.RaiseOnPaymentCompleted", 
