...
namespace NssIT.Kiosk.Sqlite.DB.AccessDB.Works
{
	...
	/// ClassCode:EXIT25.22
	/// </summary>
    public class UpSertLastCardSettlementSuccessTimeAx<Echo> : IDBAxExecution<Echo>, IDisposable
        where Echo : SuccessXEcho
    {
        ...
        ///// Parameters (_prm)
        private DateTime _prmLastSettlementSuccessTime = DateTime.Now;
        ///// xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx

        public UpSertLastCardSettlementSuccessTimeAx(DateTime lastSettlementSuccessTime)
        {
            _prmLastSettlementSuccessTime = lastSettlementSuccessTime;
        }

        /// <summary>
        /// FuncCode:EXIT25.2202
        /// </summary>
        public void DBExecute(DatabaseAx dbAx)
        {
            ...
            try
            {
                ...
                using (KioskStatesDBTrans dbTrans = new KioskStatesDBTrans())
                {
                    ...
                    // ..
                    //-------------------------------------------------------
                    dbTrans.UpSertLastSettlementSuccessTime(_prmLastSettlementSuccessTime, conn, trn);
                }
            ...
            catch (Exception ex)
            {
                ...
                ResultStatus.SetTransFail(new Exception($@"{ex?.Message}; (EXIT25.2202.EX01)", ex));
            }
            finally
            {
                trn = null;
