		...
		private async void LogInfoToDb()
		{
			...
			while (_disposed == false)
			{
				try
				{
					...
					if (inforMsg != null)
					{
						if (inforMsg.MsgXObj is NetMessagePack nMsg)
						{
							if (nMsg.MsgObject?.GetMsgData() is IAgreeLog argLog)
							{
								if (argLog.IsAgreedLog)
									WriteToTable(inforMsg);
							}
							else if (nMsg.MsgObject is IAgreeLog argLog2)
							{
								if (argLog2.IsAgreedLog)
									WriteToTable(inforMsg);
							}
							else
								WriteToTable(inforMsg);
						}
						else if (inforMsg.MsgXObj is IAgreeLog argLog3)
						{
							if (argLog3.IsAgreedLog)
								WriteToTable(inforMsg);
						}
						else
							WriteToTable(inforMsg);
			...
			if (_disposed == false)
			{
				try
				{
					...
					lock (_logList)
					{
						if (_logList.Count == 0)
						{
							Monitor.Wait(_logList, _MaxWaitPeriod);
						}
