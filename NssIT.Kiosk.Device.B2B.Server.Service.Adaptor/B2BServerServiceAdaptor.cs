...
namespace NssIT.Kiosk.Device.B2B.Server.Service.Adaptor
{
	public class B2BServerServiceAdaptor : IDisposable
	{
		...
		private IUIApplicationJob _b2bJopApp;

		public B2BServerServiceAdaptor(INetMediaInterface netMediaInterface, NetInfoRepository netInfoRepo)
		{
			...
			_b2bJopApp = new B2BJobApplication();
			_b2bJopApp.OnShowResultMessage += _b2bJopApp_OnShowResultMessage;
		}
		...
		private void _netInterface_OnDataReceived(object sender, DataReceivedEventArgs e)
		{
			...
			_b2bJopApp.SendInternalCommand((e.ReceivedData.MsgObject?.ProcessId) ?? "-", e.ReceivedData.NetProcessId, e.ReceivedData.MsgObject);
		}

		private void _b2bJopApp_OnShowResultMessage(object sender, UIMessageEventArgs e)
		{
			try
			{
				...
				//IKioskMsg svcMsg = GetResultSvcMessage(e.MessageObj, refNetProcessId, e.MsgType.Value);
			...
			//IKioskMsg GetResultSvcMessage(IEventMessageObj evtMsgObj, Guid? refNetProcId, MessageType? msgType)
			//{
			//	if (evtMsgObj == null)
			//		return null;

			//	IKioskMsg retMsg = null;
